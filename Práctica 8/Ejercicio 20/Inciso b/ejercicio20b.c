...
void alcanza(TVertice DG[],unsigned int N, TElementoG origen,TElementoG destino, short int VV[], short int *cumple, unsigned int *cantVertices);

int main()
{
    TVertice DG[MAX_VERTICES];
    unsigned int i,N,cantVertices = 0;
    ...
    // Respuesta
    alcanza(DG,N,origen,destino,VV,&cumple,&cantVertices);
    if (cumple)
    {
...
void alcanza(TVertice DG[],unsigned int N, TElementoG origen,TElementoG destino, short int VV[], short int *cumple, unsigned int *cantVertices){
    ...
    else
    {
        ...
        while (aux != NULL && !*cumple)
        {
            ...
            {
                VV[aux->id - 65] = 1;
                alcanza(DG,N,aux->id,destino,VV,cumple,cantVertices);
            }
            aux = aux->sig;
        } 
    }
