...
void alcanza(unsigned int DG[][MAX_VERTICES],unsigned int N, unsigned int origen,unsigned int destino, short int VV[], short int *cumple);

int main()
{
    unsigned int DG[MAX_VERTICES][MAX_VERTICES], i,N;
    ...
    alcanza(DG,N,origen-65,destino-65,VV,&cumple);
    ...
    else
    {
        ...
        while (i<N && !*cumple)
        {
            if (DG[origen][i] > 0 && !VV[i]){
                VV[i] = 1;
                alcanza(DG,N,i,destino,VV,cumple);
            }
            i++;
        } 
    }
